...
class AccountTax(models.Model):
    ...
    def get_withholding_vals(self, payment_group):
        ...
        _logger.info("\n\n 73 ***** withholding_type: %s", self.withholding_type)
        ...
        #_logger.info("\n\n 86 --- withholding_type: %s", self.withholding_type) 
        ...
        if self.withholding_type == 'partner_tax':
            ...
            #_logger.info("\n\n 85 ***** amount: %s - %s - %s", amount,base_amount,alicuota)
            
        ## TODO revisar Ale - sacar el or True 
        elif self.withholding_type == 'tabla_ganancias' or True:
            ...
            # TODO validar excencion actualizada
                ...
                _logger.info("\n\n b_139 ***** base_amount: %s", base_amount)
            ...
            #raise UserError(f" {vals['withholding_non_taxable_amount']} - {vals['period_withholding_amount']} - {vals}")
    ...
    def get_partner_alicuot(self, partner, date):
        ...
        if not alicuot and \
            ...
            if arba_tag and arba_tag.id in invoice_tags.ids:
                ...
                self._mensaje_agip_error(f" - No se pudo obtener la Alícuota AGIP para el cálculos de Percepciones de este proveedor: {partner.name} ", False)
